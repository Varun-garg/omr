...
#if defined(OMR_VALGRIND_MEMCHECK)
	uintptr_t childSize = extensions->objectModel.getConsumedSizeInBytesWithHeader(childEntry->objPtr);

	VALGRIND_MAKE_MEM_DEFINED(parentEntry->objPtr,size);
	VALGRIND_MAKE_MEM_DEFINED(childEntry->objPtr,childSize);
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
...
#if defined(OMR_VALGRIND_MEMCHECK)
		VALGRIND_MAKE_MEM_DEFINED(childSlot,sizeof(GC_SlotObject));
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
...
#if defined(OMR_VALGRIND_MEMCHECK)
		VALGRIND_MAKE_MEM_NOACCESS(childSlot,sizeof(GC_SlotObject));
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
	} else {
...
#if defined(OMR_VALGRIND_MEMCHECK)
	VALGRIND_MAKE_MEM_NOACCESS(parentEntry->objPtr,size);
	VALGRIND_MAKE_MEM_NOACCESS(childEntry->objPtr,childSize);
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
...
GCConfigTest::removeObjectFromRootTable(const char *name)
{
	...
	RootEntry *rootEntry = (RootEntry *)hashTableFind(exampleVM->rootTable, &searchEntry);
		...
		{
			MM_GCExtensionsBase *extensions = (MM_GCExtensionsBase *)exampleVM->_omrVM->_gcOmrVMExtensions;
			objectSize = extensions->objectModel.getConsumedSizeInBytesWithHeader(rootEntry->rootPtr);
			VALGRIND_MAKE_MEM_DEFINED(rootEntry->rootPtr,objectSize);
		}
	...
	if(NULL != rootEntry)
	{
		VALGRIND_MAKE_MEM_NOACCESS(rootEntry->rootPtr,objectSize);
	}
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
...
#if defined(OMR_VALGRIND_MEMCHECK)
	//needed declaration to prevent crosses initialization generated by goto
	uintptr_t objSize;
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
...
#if defined(OMR_VALGRIND_MEMCHECK)
	objSize = extensions->objectModel.getConsumedSizeInBytesWithHeader(objEntry->objPtr);
	VALGRIND_MAKE_MEM_DEFINED(objEntry->objPtr,objSize);
	VALGRIND_MAKE_MEM_DEFINED(parentEntry->objPtr,size);
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
...
#if defined(OMR_VALGRIND_MEMCHECK)
	VALGRIND_MAKE_MEM_NOACCESS(objEntry->objPtr,objSize);
	VALGRIND_MAKE_MEM_NOACCESS(parentEntry->objPtr,size);
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
	if (0 != rt) {
