...
public:
	...
	allocateAndInitializeObject(OMR_VMThread *omrVMThread)
	{
		...
		if (isAllocatable()) {
			...
			if (NULL != heapBytes) {
#if defined(OMR_VALGRIND_MEMCHECK)
				valgrindMempoolAlloc(env->getExtensions()->valgrindMempoolAddr, env->getExtensions()->
					_allocatedObjects,(uintptr_t) heapBytes, _allocateDescription.getBytesRequested());
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
